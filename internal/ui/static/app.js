...
`),e.lastDisplayStatus?`Last known status: ${e.lastDisplayStatus}`:``]);return`
      <button class="partition-list-item ${t?`active`:``}" data-partition="${a(e.name)}">
        <div class="partition-list-title">
          ...
          ${V(e.health,e.displayStatus,`${e.name} status`,n,`partition:${e.name}`)}
        </div>
    ...
    `}).join(``),e.querySelectorAll(`[data-partition]`).forEach(e=>{e.addEventListener(`click`,()=>D(e.dataset.partition).catch($))})}function M(){let e=v.detail,t=document.getElementById(`heroContent`);if(!t)return;if(v.activePanel!==`overviewPanel`){B();return}if(!e){t.className=`loading-state text-sm text-[#566778]`,t.textContent=`Select a partition to inspect its current shape.`,[`intentCards`,`attentionAssetsList`,`serviceHealthCards`,`recentEventsList`].forEach(e=>{let t=document.getElementById(e);t&&(t.className=`loading-state text-sm text-[#566778]`,t.textContent=`Choose a partition.`)}),B();return}let n=e.health??{},r={...e.partition?.manifest?.metadata?.labels??{},...e.partition?.manifest?.spec?.labels??{}};t.className=``,t.innerHTML=`
    <div class="hero-grid">
      <div class="hero-main">
        <div class="pill-row mb-2">
          ${V(n.status,n.displayStatus)}
  ...
  `,Oe(),F(),Le(),Be(),B()}function N(e,t){return`
    <div class="stat-card rounded-lg border border-white/[0.09]">
      <div class="stat-label">${i(e)}</div>
      <div class="stat-value">${t}</div>
    </div>
  `}function Oe(){let e=document.getElementById(`attentionAssetsList`);if(!e)return;let t=ke(),n=Ae();if(!t.length&&!n.length){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`No assets need attention right now.`;return}e.className=`attention-asset-list`,e.innerHTML=t.map(({intent:e,asset:t})=>{let n=P(t);return`
    <article class="attention-asset-card attention-asset-card-${a(t.health)}">
      <div class="attention-asset-card-header">
        ...
        ${V(t.health,t.displayStatus,`${e.name} / ${t.name}`,t.summary,`asset:${v.selectedPartition}:${e.name}:${t.name}`)}
      </div>
  ...
  `}).join(``)+(n.length?`
    <div class="progressing-asset-list mt-2">
      ...
      ${n.map(({intent:e,asset:t})=>{let n=P(t);return`
  ...
  `:``)}function ke(){return(v.detail?.intents??[]).flatMap(e=>(e.assets??[]).map(t=>({intent:e,asset:t}))).filter(({asset:e})=>e?.health===`failing`||e?.health===`attention`||e?.health===`drifted`||e?.health===`drifted-locked`).sort((e,t)=>{let n=je(e.asset.health)-je(t.asset.health);return n===0?e.intent.name===t.intent.name?e.asset.name.localeCompare(t.asset.name):e.intent.name.localeCompare(t.intent.name):n})}function Ae(){return(v.detail?.intents??[]).flatMap(e=>(e.assets??[]).map(t=>({intent:e,asset:t}))).filter(({asset:e})=>e?.health===`pending`).sort((e,t)=>e.intent.name===t.intent.name?e.asset.name.localeCompare(t.asset.name):e.intent.name.localeCompare(t.intent.name))}function P(e){let t=String(e?.summary??``).trim(),n=String(e?.observedHealth?.summary??``).trim(),r=String(e?.status??``);return(r===`Drifted`||r===`DriftedLocked`)&&n?t.includes(n)?t:t?`${t}: ${n}`:n:t}function je(e){return e===`failing`?0:e===`drifted-locked`?1:e===`drifted`?2:e===`attention`?3:4}function F(){let e=document.getElementById(`intentCards`);if(!e)return;let t=v.detail?.intents??[];if(!t.length){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`No intents defined for this partition yet.`;return}e.className=`intent-stack`,e.innerHTML=t.map(e=>{let t=xt(e.assets??[]),n=v.activityDrawer.intentName===e.name,r=St(e.assets??[]).map(t=>`
      <section class="intent-asset-group">
        <div class="intent-asset-group-title">
          <span class="intent-lane-group-dot" style="background:${bt(t.category)}"></span>
          <span>${i(t.category)} · ${t.assets.length}</span>
        </div>
        <div class="asset-grid">
          ${t.assets.map(t=>{let n=P(t),r=Me(e.name,t.name),o=v.expandedAssetKey===r,s=Z(t.type),c=`asset-detail-${I(r)}`,l=[...t.quickFacts??[]].sort((e,t)=>e.label===`Release`?-1:+(t.label===`Release`)).map(e=>`<span class="fact${mt(e.label)}" title="${a(ht(e.label))}">${i(e.label)}: ${i(e.value)}</span>`).join(``),u=o?X(t,{limit:2**53-1,truncateAt:160}):``,d=o?_t(t.outputs??{},[],{limit:2**53-1,truncateAt:160}):``,f=o?(t.references??[]).map(e=>`<span class="fact">${i(e)}</span>`).join(``):``,p=o&&(t.dependsOn??[]).length?(t.dependsOn??[]).map(e=>`<span class="fact">${i(e)}</span>`).join(``):``;return`
              <article
      ...
      <article class="intent-card">
        <div class="intent-card-header">
          <div>
            ...
            <div class="pill-row mt-2">
              ${V(e.health,e.displayStatus,`${e.name} intent`,e.summary,`intent:${v.selectedPartition}:${e.name}`)}
              ${(()=>{let t=Ze(e);return V(t.status,t.label,`${e.name} snapshot freshness`,t.detail,`intent-freshness:${v.selectedPartition}:${e.name}`)})()}
              <span class="pill">${i(e.targetSummary??`Unassigned`)}</span>
    ...
    `}).join(``),e.querySelectorAll(`[data-activity-intent]`).forEach(e=>{e.addEventListener(`click`,()=>Fe(e.dataset.activityIntent).catch($))}),e.querySelectorAll(`[data-asset-toggle]`).forEach(e=>{e.addEventListener(`click`,()=>Ne(e.dataset.assetToggle??``)),e.addEventListener(`keydown`,t=>{t.key!==`Enter`&&t.key!==` `||(t.preventDefault(),Ne(e.dataset.assetToggle??``))})})}function Me(e,t){return`${e}::${t}`}function I(e){return e.replace(/[^a-zA-Z0-9_-]+/g,`-`)}function Ne(e){e&&(v.expandedAssetKey=v.expandedAssetKey===e?``:e,F())}function Pe(e){if(!e)return;v.expandedAssetKey=e,F();let t=I(e);requestAnimationFrame(()=>{document.querySelector(`[data-asset-card="${t}"]`)?.scrollIntoView({behavior:`smooth`,block:`center`,inline:`nearest`})})}async function Fe(e){if(v.activityDrawer.intentName===e){v.activityDrawer={intentName:``,data:null,loading:!1,error:``},F();return}v.activityDrawer={intentName:e,data:null,loading:!0,error:``},F();try{let n=v.selectedPartition;v.activityDrawer={intentName:e,data:await t(`/api/partitions/${encodeURIComponent(n)}/intents/${encodeURIComponent(e)}/activity`),loading:!1,error:``}}catch(t){v.activityDrawer={intentName:e,data:null,loading:!1,error:t.message??`Failed to load activity`}}F()}function Ie(){let{data:e,loading:t,error:n}=v.activityDrawer;if(t)return`<div class="activity-drawer"><div class="activity-loading">Loading activity…</div></div>`;if(n)return`<div class="activity-drawer"><div class="activity-error">${i(n)}</div></div>`;if(!e)return`<div class="activity-drawer"><div class="activity-loading">No activity data.</div></div>`;let r=e.timestamps??{},o=[{label:`Queued`,value:r.lastQueuedAt},{label:`Check`,value:r.lastCheckAt},{label:`Diff`,value:r.lastDiffAt},{label:`Apply`,value:r.lastApplyAt}].filter(e=>e.value&&e.value!==`0001-01-01T00:00:00Z`),s=e.logs??[],l=e.drift;return`
  ...
  `}function Le(){let e=document.getElementById(`serviceHealthCards`);if(!e)return;let t=v.detail?.health?.services??[];if(!t.length){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`No service-like assets to score yet.`;return}let n=t.filter(e=>e.status===`healthy`).length,r=t.filter(e=>e.status===`attention`).length,o=t.filter(e=>e.status===`failing`).length,s=t.filter(e=>e.taskActive).length,c=t.filter(e=>e.taskTimedOut).length;e.className=`service-stack`,e.innerHTML=`
    ...
    <article class="service-card service-card-${a(e.status??`neutral`)}">
      <div class="service-card-header">
        ...
        ${V(e.status,e.displayStatus,`${e.intent} / ${e.asset}`,e.summary,`service:${v.selectedPartition}:${e.intent}:${e.asset}`)}
      </div>
      <p class="service-card-note">${i(Re(e))}</p>
      <div class="service-health-meta">
        ${ze(e)}
      </div>
      <div class="service-card-actions">
        <button class="btn-secondary service-card-action" type="button" data-service-focus="${a(Me(e.intent,e.asset))}">Open details</button>
      </div>
    </article>
  `).join(``)}
  `,e.querySelectorAll(`[data-service-focus]`).forEach(e=>{e.addEventListener(`click`,()=>Pe(e.dataset.serviceFocus??``))})}function Re(e){if(e.taskTimedOut)return`Last reconcile task timed out. Open details in the intent map for config and outputs.`;if(e.taskActive)return`Reconcile is currently running for this service.`;switch(e.status){case`healthy`:return`Operational summary only. Configuration and ports stay in the intent map.`;case`pending`:return`Waiting for the first successful reconcile.`;case`attention`:return String(e.summary??`Needs attention.`);case`failing`:return String(e.summary??`Service is failing.`);default:return String(e.summary??`Service status unavailable.`)}}function ze(e){let t=[];e.taskActive&&t.push(`reconcile running`),e.taskTimedOut&&t.push(`last task timed out`);let n=c(e.lastUpdatedAt);return n!==`—`&&t.push(`updated ${n}`),t.map(e=>`<span class="service-health-meta-item">${i(e)}</span>`).join(``)}function Be(){let e=document.getElementById(`recentEventsList`);if(!e)return;let t=v.detail?.recentEvents??[];if(!t.length){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`Recent event history loads only in the History tab.`;return}e.className=`timeline-stack`,e.innerHTML=Ge(t).map(Ke).join(``)}function L(){let e=document.getElementById(`deploymentTimeline`),t=document.getElementById(`eventTimeline`);if(!e||!t)return;if(!v.selectedPartition){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`Select a partition to inspect deployment history.`,t.className=`empty-state text-sm text-[#566778]`,t.textContent=`Select a partition to inspect event history.`;return}if(v.historyLoading){e.className=`loading-state text-sm text-[#566778]`,e.textContent=`Loading deployment history…`,t.className=`loading-state text-sm text-[#566778]`,t.textContent=`Loading event history…`;return}if(v.historyError){e.className=`empty-state text-sm text-[#566778]`,e.textContent=v.historyError,t.className=`empty-state text-sm text-[#566778]`,t.textContent=v.historyError;return}let n=v.history;if(!n){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`Open the History tab to load deployment history.`,t.className=`empty-state text-sm text-[#566778]`,t.textContent=`Open the History tab to load event history.`;return}let r=(document.getElementById(`historyFilter`)?.value??``).trim().toLowerCase(),i=(n.deployments??[]).filter(e=>r?`${e.intent} ${e.deploymentRevision} ${(e.assets??[]).map(e=>`${e.asset} ${e.version??``}`).join(` `)}`.toLowerCase().includes(r):!0),a=(n.events??[]).filter(e=>r?`${e.intent??``} ${e.title??``} ${e.message??``}`.toLowerCase().includes(r):!0);e.className=i.length?`timeline-stack`:`empty-state text-sm text-[#566778]`,e.innerHTML=i.length?i.map(qe).join(``):`No deployment entries match the current filter.`;let o=document.getElementById(`historyGroupToggle`),s=!o||o.checked?Ge(a):a;t.className=s.length?`timeline-stack`:`empty-state text-sm text-[#566778]`,t.innerHTML=s.length?s.map(Ke).join(``):`No events match the current filter.`}function R(){let e=document.getElementById(`rolloutsTimeline`);if(!e)return;if(!v.selectedPartition){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`Select a partition to inspect rollout history.`;return}if(v.rolloutsLoading){e.className=`loading-state text-sm text-[#566778]`,e.textContent=`Loading rollouts…`;return}if(v.rolloutsError){e.className=`empty-state text-sm text-[#566778]`,e.textContent=v.rolloutsError;return}let t=v.rollouts?.rollouts??[];e.className=t.length?`timeline-stack`:`empty-state text-sm text-[#566778]`,e.innerHTML=t.length?t.map(Ve).join(``):`No archived rollouts were found for this partition yet.`,e.querySelectorAll(`[data-rollout-toggle]`).forEach(e=>{e.addEventListener(`click`,t=>{t.preventDefault();let n=e.dataset.rolloutToggle??``;n&&He(n)})})}function Ve(e){let t=e.assets??[],n=t.length,r=Ue(e),s=!!v.expandedRolloutKeys[r],l=e.current?V(`healthy`,`Current`):e.newIntent?V(`pending`,`New intent`):V(`healthy`,`Rollout`);return`
          ...
          ${t.map(t=>`
            <div class="timeline-asset">
              <div class="flex justify-between items-start gap-2">
                ...
                ${V(We(t.change),o(t.change||`updated`))}
              </div>
  ...
  `}function He(e){e&&(v.expandedRolloutKeys={...v.expandedRolloutKeys,[e]:!v.expandedRolloutKeys[e]},R())}function Ue(e){return`${e.intent??``}::${e.deploymentRevision??``}`}function We(e){switch((e??``).toLowerCase()){case`added`:return`pending`;case`removed`:return`attention`;default:return`healthy`}}function Ge(e){let t=new Map;for(let n of e){let e=`${n.intent??``}::${n.title}`,r=t.get(e);!r||new Date(n.timestamp)>new Date(r.latest.timestamp)?t.set(e,{latest:n,count:(r?.count??0)+1}):r.count++}return Array.from(t.values()).sort((e,t)=>new Date(t.latest.timestamp).getTime()-new Date(e.latest.timestamp).getTime()).map(e=>({...e.latest,groupCount:e.count}))}function Ke(e){let t=e.groupCount>1?`<span class="event-count-pill" title="${e.groupCount} occurrences">${e.groupCount}×</span>`:``,n=(e.title??``).toLowerCase().replace(/[^a-z0-9]/g,``),r=(e.message??``).toLowerCase().replace(/[^a-z0-9]/g,``),a=e.message&&r!==n;return`
    <article class="timeline-card">
      <div class="timeline-head">
        <div>
        ...
        ${V(e.status,e.displayStatus,e.title??`Event`,e.message??``)}
      </div>
      <div class="timeline-meta">
  ...
  `}function z(){let e=document.getElementById(`topologyCanvas`);if(!e)return;let t=v.detail?.topology;_(document.getElementById(`topologyLegend`)),g({canvas:e,topology:t,zoom:v.topology.zoom,savedPositions:v.topology.nodePositions,selectedNodeId:v.topology.selectedNodeId,filters:{contains:document.getElementById(`showContainEdges`)?.checked??!0,join:document.getElementById(`showJoinEdges`)?.checked??!0,dependsOn:document.getElementById(`showAssetEdges`)?.checked??!0,outputRef:document.getElementById(`showOutputEdges`)?.checked??!0},onSelectNode:(e,t)=>{v.topology.selectedNodeId=e,v.topology.nodePositions=t,Je(),z()},onDragNode:(e,t)=>{v.topology.nodePositions=t}}),Je()}function Je(){let e=document.getElementById(`topologyDetails`);if(!e)return;let t=v.detail?.topology;if(!t?.nodes?.length){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`Select a node to inspect its status, metadata, and linked details.`;return}let n=new Map(t.nodes.map(e=>[e.id,e])),r=n.get(v.topology.selectedNodeId);if(!r){e.className=`empty-state text-sm text-[#566778]`,e.textContent=`Select a node to inspect its status, metadata, and linked details.`;return}let a={contains:document.getElementById(`showContainEdges`)?.checked??!0,join:document.getElementById(`showJoinEdges`)?.checked??!0,dependsOn:document.getElementById(`showAssetEdges`)?.checked??!0,outputRef:document.getElementById(`showOutputEdges`)?.checked??!0},s=(t.edges??[]).filter(e=>a[e.kind]!==!1).filter(e=>e.from===r.id||e.to===r.id),c=r.kind===`asset`?st(r.intent,r.asset??r.label):null,l=r.kind===`intent`?ot(r.intent??r.label):null,u=c?X(c):``,d=_t(c?.outputs??l?.outputs??{},c?[]:l?.outputHints??[]),f=(c?.references??[]).map(e=>`<span class="fact">${i(e)}</span>`).join(``),p=s.map(e=>{let t=e.from===r.id?e.to:e.from,a=n.get(t);return a?`
    ...
    <div style="--node-accent:${ct(r)}">
      <div class="topology-detail-header">
        <div class="topology-detail-icon">${i(lt(r))}</div>
        <div>
          <h3>${i(r.label)}</h3>
          <p>${i(ut(r))}</p>
        </div>
      ...
      <div class="pill-row mb-2">
        ${V(r.health??r.status,r.displayStatus??o(r.kind),r.label,r.description,`topology:${v.selectedPartition}:${r.id}`)}
  ...
  `}var Ye={overviewPanel:{eyebrow:`Control center`,title:`Control Center`},topologyPanel:{eyebrow:`Deployment graph`,title:`Topology`},rolloutsPanel:{eyebrow:`Release timeline`,title:`Rollouts`},historyPanel:{eyebrow:`Push timeline`,title:`History`}};function B(){let e=v.activePanel,t=v.detail,r=Ye[e]??Ye.overviewPanel,a=e===`overviewPanel`,o=!!v.selectedPartition,s=a&&!o,c=(e,t)=>{let n=document.getElementById(e);n&&(n.style.display=t?``:`none`)};c(`appGridSection`,s),c(`summaryGrid`,s),c(`selectedPartitionHero`,a&&o),c(`sidebarPartitionSection`,!0),n(`pageEyebrow`,r.eyebrow),n(`pageTitle`,r.title);let l=`Monitor partitions, inspect topology, and review history.`,u=``;if(e===`overviewPanel`&&t){l=`${t.partition.manifest.spec?.deletionPolicy??`orphan`} policy · ${t.partition.manifest.spec?.reconciliation?.mode??`manual`} reconcile · ${t.intents.length} intents`;let e=Qe(t),n=V(e.status,e.label,`${t.partition.manifest.metadata.name} snapshot freshness`,e.detail,`partition-freshness:${t.partition.manifest.metadata.name}`);u=`${V(t.health?.status,t.health?.displayStatus??`Selected`,`${t.partition.manifest.metadata.name} health`,t.health?.summary,`partition-health:${t.partition.manifest.metadata.name}`)} ${n} <span class="pill">${t.topology?.nodes?.length??0} nodes</span>`}e===`topologyPanel`&&(l=t?`Topology for ${t.partition.manifest.metadata.name}.`:`Select a partition to inspect its graph.`,u=t?`<span class="pill">${i(t.partition.manifest.metadata.name)}</span><span class="pill">${t.topology?.nodes?.length??0} nodes</span>`:``),e===`rolloutsPanel`&&(l=t?`Review archived rollout changes for ${t.partition.manifest.metadata.name}.`:`Select a partition to inspect rollout history.`,u=t?`<span class="pill">${i(t.partition.manifest.metadata.name)}</span><span class="pill">${v.rollouts?.rollouts?.length??0} rollouts</span>`:``),e===`historyPanel`&&(l=t?`Review deployments and state changes for ${t.partition.manifest.metadata.name}.`:`Select a partition to inspect history.`,u=t?`<span class="pill">${i(t.partition.manifest.metadata.name)}</span><span class="pill">${v.history?.deployments?.length??0} deployments</span>`:``),n(`pageSubtitle`,l);let d=document.getElementById(`headerContextPills`);d&&(d.innerHTML=u,d.style.display=u.trim()?``:`none`);let f=document.getElementById(`topnavPartition`);f&&(f.textContent=v.selectedPartition||t?.partition?.manifest?.metadata?.name||r.title||`Control Center`)}function V(e,t,n,r,s){let c=String(e??`neutral`).toLowerCase(),l=t??o(c),u=$e(s,c,r);return(c===`failing`||c===`attention`||c===`drifted`||c===`drifted-locked`)&&u.length>0?`<button type="button" class="badge badge-${a(c)} badge-clickable" data-diagnostic-title="${a((n??l).trim())}" data-diagnostic-detail="${a(u)}" aria-label="Show diagnostic details for ${a(l)}">${i(l)}</button>`:`<span class="badge badge-${a(c)}">${i(l)}</span>`}function H(e){if(!e)return null;let t=Date.parse(String(e));return Number.isFinite(t)?t:null}function Xe(e){let t=e?.state?.timestamps??{},n=[H(t.lastCheckAt),H(t.lastApplyAt),H(t.lastDiffAt),H(t.lastQueuedAt)].filter(e=>e!==null);return n.length===0?null:Math.max(...n)}function U(e){let t=Math.max(0,Math.floor(e/1e3));if(t<60)return`${t}s ago`;let n=Math.floor(t/60);if(n<60)return`${n}m ago`;let r=Math.floor(n/60);return r<24?`${r}h ago`:`${Math.floor(r/24)}d ago`}function Ze(e){let t=Xe(e);if(t===null)return{status:`neutral`,label:`No sample`,detail:`No reconciliation timestamps are available yet for this intent.`};let n=Date.now()-t,r=n>ue;return{status:r?`attention`:`healthy`,label:r?`Stale ${U(n)}`:`Fresh ${U(n)}`,detail:`Last sampled ${U(n)} at ${c(new Date(t).toISOString())}.`}}function Qe(e){let t=(Array.isArray(e?.intents)?e.intents:[]).map(e=>Xe(e)).filter(e=>e!==null);if(t.length===0)return{status:`neutral`,label:`No samples`,detail:`No intent reconciliation timestamps are available for this partition.`};let n=Math.max(...t),r=Date.now()-n,i=r>ue;return{status:i?`attention`:`healthy`,label:i?`Stale snapshot ${U(r)}`:`Fresh snapshot ${U(r)}`,detail:`Most recent intent sample is ${U(r)} old (${c(new Date(n).toISOString())}).`}}function $e(e,t,n){let r=String(e??``).trim(),i=String(n??``).trim();return r?t===`failing`||t===`attention`||t===`drifted`||t===`drifted-locked`?i?(v.diagnosticDetails[r]=i,i):v.diagnosticDetails[r]??``:(delete v.diagnosticDetails[r],i):i}function et(e){return e.map(e=>String(e??``).trim()).filter(e=>e.length>0).join(`
`)}function tt(){let e=document.getElementById(`diagnosticsModal`);if(e)return e;let t=document.createElement(`div`);return t.id=`diagnosticsModal`,t.className=`diagnostics-modal hidden`,t.innerHTML=`
  ...
  `,t.addEventListener(`click`,e=>{let n=e.target;(n===t||n.closest(`[data-diagnostics-close='true']`))&&rt()}),document.body.appendChild(t),t}function nt(e,t){let n=tt(),r=n.querySelector(`#diagnosticsModalTitle`),i=n.querySelector(`#diagnosticsModalBody`);r&&(r.textContent=e.trim()||`Status details`),i&&(i.textContent=t.trim()),n.classList.remove(`hidden`),document.body.classList.add(`diagnostics-open`)}function rt(){let e=document.getElementById(`diagnosticsModal`);e&&(e.classList.add(`hidden`),document.body.classList.remove(`diagnostics-open`))}function it(e){let t=e.partition?.manifest?.spec?.reconciliation?.mode??`manual`,n=e.partition?.manifest?.spec?.labels?.managedBy??``,r=(e.intents??[]).some(e=>e.targetSummary&&e.targetSummary!==`Unassigned`),i=e.health?.pending??0,a=[];return a.push(`${i} asset${i===1?` is`:`s are`} in <strong>Planned</strong> state — no reconcile has run yet.`),n===`external-compose`&&a.push(`Resources in this partition are managed externally by Docker Compose.`),t===`manual`?r?a.push(`Click <strong>Reconcile now</strong> in the sidebar to run the first reconcile and deploy assets.`):a.push(`No pusher is assigned. Assets will stay in Planned state until a pusher is configured.`):a.push(`The reconciler will process these assets automatically in the next cycle.`),`<div class="info-callout mt-2"><span class="info-callout-icon">?</span><div><strong>Why is this partition Progressing?</strong><p>${a.join(` `)}</p></div></div>`}function at(e){let t=[`component`,`role`,`stack`,`topology`],n=[];return t.forEach(t=>{e[t]&&n.push(e[t])}),[...new Set(n)]}function ot(e){return(v.detail?.intents??[]).find(t=>t.name===e)??null}function st(e,t){return ot(e)?.assets?.find(e=>e.name===t)??null}var W={partition:`#F0E442`,intent:`#CC79A7`,runtime:`#0072B2`,config:`#009E73`,storage:`#56B4E9`,traffic:`#D55E00`,muted:`#8B949E`};function ct(e){return e.kind===`partition`?W.partition:e.kind===`intent`?W.intent:yt(e.assetType??e.kind)}function lt(e){return e.kind===`partition`?`◫`:e.kind===`intent`?`⊞`:vt(e.assetType??e.kind)}function ut(e){return e.kind===`partition`?`${e.meta?.reconciliation??`manual`} reconcile · ${e.meta?.deletionPolicy??`orphan`} delete`:e.kind===`intent`?e.meta?.target??e.displayStatus??`Intent`:`${Q(e.assetType??e.kind)} · ${e.displayStatus??`Asset`}`}var G=[`Compute`,`Network`,`Config`,`Storage`,`Operations`],dt={Compute:W.runtime,Volume:W.storage,Config:W.config,ObjectStore:W.storage,Database:W.traffic,SQLDatabase:W.traffic,LoadBalancer:W.traffic,Observability:W.config},ft={Image:`Container image reference (registry/name:tag@digest)`,Scale:`Desired replica count`,Env:`Environment variables injected at runtime`,Config:`ConfigMap or file mounts`,Storage:`Persistent volume mounts`,Ports:`Exposed service ports`,Port:`Service listener port`,Health:`Health check probe is configured`,CPU:`CPU resource limit/request`,Memory:`Memory resource limit/request`,Engine:`Storage engine or database type`,Version:`Engine version`,Database:`Database name`,Mode:`Deployment or storage mode`,Endpoint:`Connection endpoint address`,Size:`Volume storage capacity`,Access:`Volume access mode (e.g. ReadWriteOnce)`,Format:`Config file format (yaml / json / env)`,Files:`Config file definitions`,Inline:`Config data is defined inline in the manifest`,Targets:`Number of load balancer backend targets`,Listeners:`Number of load balancer listeners`,Buckets:`Object storage bucket names`,Provider:`Observability provider`,Receivers:`Telemetry input protocols`,Exporters:`Telemetry export destinations`,Outputs:`Output keys exposed to dependent intents`},pt={Scale:`fact-scale`,Ports:`fact-port`,Port:`fact-port`,CPU:`fact-resource`,Memory:`fact-resource`,Env:`fact-env`,Storage:`fact-storage`,Size:`fact-storage`,Engine:`fact-engine`,Version:`fact-engine`,Outputs:`fact-outputs`};function mt(e){return e===`Release`?`fact-release`:pt[e]?` ${pt[e]}`:``}function ht(e){return ft[e]??e}function K(e){return(v.catalog?.assetTypes??[]).find(t=>t.type===e)??null}function q(e){return e.replace(/\[\d+\]/g,`[]`)}function J(e,t){let n=q(t),r=n.replace(/\[\]/g,``).split(`.`)[0];return(e??[]).find(e=>e.path===n||e.path===r)??null}function gt(e,t){let n=K(e?.type??``);return J(e?.hints,t)??J(n?.hints,t)??J(n?.fields,t)??null}function Y(e,t=``){if(e==null||e===``)return[];if(Array.isArray(e))return e.length?e.flatMap((e,n)=>Y(e,`${t}[${n}]`)):t?[{path:t,value:`[]`}]:[];if(typeof e==`object`){let n=Object.entries(e);return n.length?n.flatMap(([e,n])=>Y(n,t?`${t}.${e}`:e)):t?[{path:t,value:`{}`}]:[]}return t?[{path:t,value:String(e)}]:[]}function X(e,t={}){let n=t.limit??16,r=t.truncateAt??48,o=K(e?.type??``),c=new Map;(o?.fields??[]).forEach((e,t)=>{c.set(e.path,t)});let l=Y(e?.properties??{}).sort((e,t)=>{let n=q(e.path),r=q(t.path),i=n.replace(/\[\]/g,``).split(`.`)[0],a=r.replace(/\[\]/g,``).split(`.`)[0],o=c.get(n)??c.get(i)??2**53-1,s=c.get(r)??c.get(a)??2**53-1;return o===s?e.path.localeCompare(t.path):o-s});if(!l.length)return``;let u=l.slice(0,n),d=u.map(t=>{let n=gt(e,t.path);return`<span class="fact" title="${a([t.path,n?.title,n?.description].filter(Boolean).join(` - `))}">${i(`${t.path}: ${s(t.value,r)}`)}</span>`}).join(``);if(l.length===u.length)return d;let f=l.length-u.length;return`${d}<span class="fact" title="${f} more properties available">+${f} more</span>`}function _t(e,t=[],n={}){let r=Object.entries(e??{}),o=n.limit??r.length,c=n.truncateAt??2**53-1,l=r.slice(0,o).map(([e,n])=>{let r=J(t,`outputs.${e}`);return`<span class="fact" title="${a([`outputs.${e}`,r?.title,r?.description].filter(Boolean).join(` - `))}">${i(`${e}: ${s(String(n),c)}`)}</span>`}).join(``);if(r.length<=o)return l;let u=r.length-o;return`${l}<span class="fact" title="${u} more outputs available">+${u} more</span>`}function Z(e){return K(e)?.category??`Other`}function Q(e){return K(e)?.title??o(e)}function vt(e){return K(e)?.icon??`⬡`}function yt(e){return dt[e]??W.muted}function bt(e){let t={Compute:W.runtime,Network:W.traffic,Config:W.config,Storage:W.storage,Operations:W.config,Other:W.muted};return t[e]??t.Other}function xt(e){let t=new Map;return e.forEach(e=>{let n=Z(e.type);t.set(n,(t.get(n)??0)+1)}),[...t.keys()].sort((e,t)=>{let n=G.indexOf(e),r=G.indexOf(t);return n===-1&&r===-1?e.localeCompare(t):n===-1?1:r===-1?-1:n-r}).map(e=>({category:e,count:t.get(e)}))}function St(e){let t=new Map;return e.forEach(e=>{let n=Z(e.type);t.has(n)||t.set(n,[]),t.get(n).push(e)}),[...t.keys()].sort((e,t)=>{let n=G.indexOf(e),r=G.indexOf(t);return n===-1&&r===-1?e.localeCompare(t):n===-1?1:r===-1?-1:n-r}).map(e=>({category:e,assets:t.get(e).sort((e,t)=>e.name.localeCompare(t.name))}))}function Ct(){document.querySelectorAll(`[data-tab-target]`).forEach(e=>{e.addEventListener(`click`,()=>ge(e.dataset.tabTarget))}),document.getElementById(`partitionSearch`)?.addEventListener(`input`,Ee),document.getElementById(`refreshButton`)?.addEventListener(`click`,()=>E(!0).catch($)),document.getElementById(`reconcileButton`)?.addEventListener(`click`,Tt),document.getElementById(`createPartitionButton`)?.addEventListener(`click`,()=>d(`Create partition via guardianctl partition push`,`success`)),document.getElementById(`overviewCreatePartitionButton`)?.addEventListener(`click`,()=>d(`Create partition via guardianctl partition push`,`success`)),document.getElementById(`appGridSearch`)?.addEventListener(`input`,De),document.getElementById(`historyFilter`)?.addEventListener(`input`,L),document.getElementById(`historyGroupToggle`)?.addEventListener(`change`,L),document.getElementById(`historyApply`)?.addEventListener(`click`,()=>Se().catch($)),document.getElementById(`historyReset`)?.addEventListener(`click`,()=>Ce().catch($)),document.getElementById(`refreshRolloutsButton`)?.addEventListener(`click`,()=>{A(!0).catch($),T()}),[`showContainEdges`,`showJoinEdges`,`showAssetEdges`,`showOutputEdges`].forEach(e=>{document.getElementById(e)?.addEventListener(`change`,z)}),document.getElementById(`topologyZoomOut`)?.addEventListener(`click`,()=>Et(-.1)),document.getElementById(`topologyZoomIn`)?.addEventListener(`click`,()=>Et(.1)),document.getElementById(`topologyResetLayout`)?.addEventListener(`click`,()=>{v.topology.nodePositions={},z()}),document.addEventListener(`click`,e=>{let t=e.target.closest(`[data-diagnostic-detail]`);t&&(e.preventDefault(),nt(t.dataset.diagnosticTitle??`Status details`,t.dataset.diagnosticDetail??`No diagnostic details were provided.`))}),document.addEventListener(`keydown`,e=>{e.key===`Escape`&&rt()}),tt(),wt()}function wt(){let e=document.getElementById(`refreshSlider`),t=document.getElementById(`refreshIntervalLabel`),n=document.getElementById(`refreshPopover`),r=document.getElementById(`syncIndicator`);if(!e||!n||!r)return;let i=Math.round(v.refreshIntervalMs/1e3);e.value=String(Math.min(b/1e3,Math.max(se/1e3,i))),t&&(t.textContent=`${e.value}s`);function a(){n.classList.contains(`hidden`)?n.classList.remove(`hidden`):n.classList.add(`hidden`)}r.addEventListener(`click`,e=>{e.stopPropagation(),a()}),e.addEventListener(`input`,()=>{let n=Number(e.value);t&&(t.textContent=`${n}s`)}),e.addEventListener(`change`,()=>{let t=Number(e.value);if(!(!Number.isFinite(t)||t<1)){v.refreshIntervalMs=t*1e3;try{localStorage.setItem(y,String(v.refreshIntervalMs))}catch{}w()}}),document.addEventListener(`click`,e=>{if(n.classList.contains(`hidden`))return;let t=e.target;!n.contains(t)&&t!==r&&n.classList.add(`hidden`)})}async function Tt(){let e=v.selectedPartition;if(!e){d(`Select a partition first.`,`error`);return}await t(`/api/partitions/${encodeURIComponent(e)}/reconcile`,{method:`POST`}),me(),d(`Reconciliation requested.`,`success`),await E(!1),await D(e,!1)}function Et(e){let t=document.getElementById(`topologyCanvas`),n=Ot(v.topology.zoom,.4,2.5),r=Ot(Math.round((n+e)*100)/100,.4,2.5);if(n===r)return;let i=t?t.scrollLeft+t.clientWidth/2:0,a=t?t.scrollTop+t.clientHeight/2:0;if(v.topology.zoom=r,z(),t){let e=r/n;t.scrollLeft=Math.max(0,i*e-t.clientWidth/2),t.scrollTop=Math.max(0,a*e-t.clientHeight/2)}Dt()}function Dt(){let e=v.topology.zoom,t=document.getElementById(`topologyZoomOut`),n=document.getElementById(`topologyZoomIn`),r=document.getElementById(`topologyZoomValue`);r&&(r.textContent=`${Math.round(e*100)}%`),t&&(t.disabled=e<=.4),n&&(n.disabled=e>=2.5)}function Ot(e,t,n){return Math.min(n,Math.max(t,e))}function $(e){d(e?.message??`Unexpected error`,`error`)}async function kt(){try{v.catalog=await t(`/api/catalog`)}catch{}}kt();